...
#Get the actual metric I want

#I want to predict recidivism within the first year
rec$recidivism_y <- rec$Recidivism_Arrest_Year1
...
print("Null Model Brier Scores")
calculate_brier(train_df)
calculate_brier(test_df)
...
for (i in 1:K_folds) {
  ...
  #Model
  lasso_model <- gamlr(X_train, y_train, family = "binomial", lambda.min.ratio = 0.0085)
...
#Train LLR Model
lasso_glm <- gamlr(lasso_x, lasso_y, family = "binomial", lambda.min.ratio = 0.0085)
...
#write.xlsx(lasso_coef_df, file = "lasso_year1.xlsx", sheetName = "Sheet1", row.names = TRUE)
...
set.seed(1994)
#Create predictions for each fold and bind them together
for (i in 1:K_folds) {
...
params <- list(
  ...
  num_leaves = 38,
  learning_rate = 0.14740225580038815,
  feature_fraction = 0.8933003775789943,
  max_depth = 2,
  boosting_type = 'gbdt'
)
...
#### Unweighted Lasso-RandomForest Model

```{r stack1}

#Prepare data for second level learner

stacked_df <- data.frame(recidivism_y = rec_df$recidivism_y, Race = rec_df$Race,
                         nb = nb_predictions, lasso = lasso_predictions, rf = rf_predictions, lgb = lgb_predictions)

stacked_val_df <- data.frame(recidivism_y = rec_val$recidivism_y, Race = rec_val$Race,
                         nb = nb_val_pred, lasso = lasso_val_pred, rf = rf_val_pred, lgb = lgb_val_pred)
```

```{r stack2}

#stacked_df <- stacked_df %>% mutate(phat = (1*lasso + 0*lgb))
#stacked_df$yhat <- ifelse(stacked_df$phat > 0.5, 1, 0) 

#The prediction probability is the average of the probabilities from the lasso and randomforest models
stacked_val_df <- stacked_val_df %>% mutate(phat = (0.4*lasso + 0.4*lgb +0.2*rf))
stacked_val_df$yhat <- ifelse(stacked_val_df$phat > 0.5, 1, 0) 
```

```{r stack3}

#Train Set
#stacked_brier <- calculate_brier(stacked_df) %>% round(5)
#cat("Unweighted Stacked Model Brier Score (lower is better):", stacked_brier, "\n")

#stacked_fp <- calculate_fairness(stacked_df)$fairness_penalty
#cat("Unweighted Stacked Model Fairness Penalty (higher is better):", stacked_fp, "\n")

#Test Set
stacked_brier <- calculate_brier(stacked_val_df) %>% round(5)
cat("Unweighted Stacked Model Brier Score (lower is better):", stacked_brier, "\n")

stacked_fp <- calculate_fairness(stacked_val_df)$fairness_penalty
cat("Unweighted Stacked Model Fairness Penalty (higher is better):", stacked_fp, "\n")
```

```{r stack4}
model_score <- calculate_score(stacked_brier, stacked_fp)
cat("Unweighted Stacked Model Score (higher is better):", model_score, "\n")
```

```{r stack5}
calculate_fairness(stacked_val_df)$confusion
```
-->
